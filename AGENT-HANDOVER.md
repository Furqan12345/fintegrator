...
**Latest:** Added persisted `ReceivedInput` and `StepPacketLog` records, redacted HTTP request/response capture, page/attempt packet metadata, clearer pagination-limit diagnostics, and shared packet viewer in Transaction/Execution Detail. Existing Amazon sample JSON was preserved unchanged. Build: 0 errors; tests: 147/147. API schema startup and `/health` verified; client build remains green.
