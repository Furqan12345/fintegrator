...
**Latest:** Added persisted `ReceivedInput` and `StepPacketLog` records, redacted HTTP request/response capture, page/attempt packet metadata, clearer pagination-limit diagnostics, and shared packet viewer in Transaction/Execution Detail. Fixed existing SQLite migration (`ReceivedInput` column) and Development sample reseed FK cleanup. Existing Amazon sample JSON preserved. Build: 0 errors; tests: 147/147. API `/health`, client, and Blazor boot return HTTP 200.
